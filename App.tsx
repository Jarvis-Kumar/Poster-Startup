...
import { 
  ...
  Zap, Brain, Video, ShoppingBag, Gamepad2, Send, AlertTriangle, CheckCircle
} from 'lucide-react';
...
const Header = () => {
  ...
  return (
    <nav className={`fixed w-full z-50 transition-all duration-500 ease-out ${scrolled ? 'bg-slate-950/80 backdrop-blur-md py-4' : 'bg-transparent pt-12 pb-8'}`}>
...
const CustomFeedbackForm = ({ onSubmit }: CustomFeedbackFormProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitted(true);
      setFeedbackText(""); // Clear text for next submission if needed
    ...
    return (
      <div className="w-full h-full min-h-[400px] flex flex-col items-center justify-center text-center p-8 bg-slate-900/50 animate-in fade-in">
        ...
        <button 
          onClick={() => {
            setSubmitted(false);
            setFeedbackText(""); // Ensure text is cleared
          }}
          className="px-10 py-4 bg-slate-800 hover:bg-slate-700 text-white rounded-full font-bold text-lg transition-all border border-slate-700 hover:border-purple-500/50 hover:shadow-lg hover:shadow-purple-500/10 flex items-center gap-2 group"
        >
          Submit Another Response <ChevronRight size={20} className="group-hover:translate-x-1 transition-transform" />
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="w-full relative">
      ...
      >
        <div>
          <label className="block text-white text-3xl md:text-5xl lg:text-7xl font-black mb-8 tracking-tight drop-shadow-lg">Comments & Reviews</label>
          <textarea 
            value={feedbackText}
            onChange={(e) => setFeedbackText(e.target.value)}
            required
            placeholder="Share your thoughts on AlgoEdit..."
            className="w-full h-64 md:h-80 bg-slate-900/80 border border-slate-800 rounded-2xl p-6 md:p-8 text-white text-lg md:text-xl placeholder-slate-600 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-all resize-none shadow-inner disabled:opacity-50"
