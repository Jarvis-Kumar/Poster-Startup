...
// Helper class to replace the prototype function for better TS compatibility
class Pointer {
  id: number = -1;
  texcoordX: number = 0;
  texcoordY: number = 0;
...
function SplashCursor({
  ...
  PRESSURE = 0.1,
  PRESSURE_ITERATIONS = 10, // Reduced from 20 to 10 for significant speedup
  CURL = 3,
  SPLAT_RADIUS = 0.2,
  SPLAT_FORCE = 13000,
  SHADING = true,
  ...
  useEffect(() => {
    ...
    if (!ext.supportLinearFiltering) {
      config.DYE_RESOLUTION = 512; // Further reduce if linear filtering is not supported
    ...
    function calcDeltaTime() {
      ...
      // Optimize: Increase cap to ~30fps step to prevent slow motion on lag
      // Was 0.016666 (60fps), updated to 0.032 for robustness
